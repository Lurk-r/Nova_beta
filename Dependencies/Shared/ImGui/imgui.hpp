...
namespace ImGui
{
    ...
    // Animation Helpers
    IMGUI_API float CalcGradientPhase(float time, float frequency = 2.0f, float offset = 0.0f);

    // Animation timing constants
    namespace AnimTiming {
        constexpr float GRADIENT_FREQ_PRIMARY = 2.0f;
        constexpr float GRADIENT_FREQ_SECONDARY = 1.8f;
        constexpr float GRADIENT_FREQ_TERTIARY = 1.5f;
        constexpr float GRADIENT_FREQ_FAST = 2.5f;
        constexpr float GRADIENT_FREQ_GLOW = 3.0f;

        // Gradient color constants for consistent animated gradients - more noticeable
        constexpr float GRAD_LIFT = 0.15f;           // Brighter start
        constexpr float GRAD_VARIANCE = 0.12f;       // More variation
        constexpr float GRAD_Y_OFFSET = 0.06f;
        constexpr float GRAD_ACCENT_LIFT = 0.18f;    // Brighter accent
        constexpr float GRAD_BASE_SCALE = 0.70f;     // Darker end for contrast
