...
bool AuthenticateUser(const std::string& username, const std::string& password, const std::string& deviceId, std::string& errorMsg) {
    //LogToFile("AuthenticateUser: Function started. Username: " + username + ", HWID: " + deviceId);
    ...
    if (httpCode != 200) {
        try {
            ...
            if (response.contains("error")) {
                ...
                // Handle specific error codes from server.py
                if (response.contains("code")) {
                    int errorCode = response["code"].get<int>();
                    switch (errorCode) {
                    case 1: // LoginStatus.expired
                        errorMsg = "Your account has expired";
                        break;
                    case 2: // LoginStatus.deviceidmissmatch
                        errorMsg = "Device ID mismatch detected";
                        break;
                    case 3: // LoginStatus.invalidpass
                        errorMsg = "Invalid password";
                        break;
                    case 4: // LoginStatus.usernotfound
                        errorMsg = "User not found";
                        break;
                    case 5: // LoginStatus.unauthorized
                        errorMsg = "Unauthorized access";
                        break;
                    case 6: // LoginStatus.servererror
                ...
                errorMsg = "Authentication failed: HTTP " + std::to_string(httpCode);
            }
        }
        catch (const json::exception& e) {
            errorMsg = "Failed to parse error response: " + std::string(e.what());
        }
        //LogToFile("AuthenticateUser: Authentication failed: " + errorMsg);
        return false;
    }

    // HTTP 200 OK - Check for success response
    try {
        json response = json::parse(readBuffer);
        if (response.contains("success") && response["success"].get<std::string>() == "Ok") {
            // Extract expiry date if available in the response
            if (response.contains("expiry")) {
                expiryDate = response["expiry"].get<std::string>();
            }
            else {
                expiryDate = "Active"; // Default if not provided
            }
            //LogToFile("AuthenticateUser: Authentication successful for user: " + username);
            return true;
        }
        else {
            // This should rarely happen - server returned 200 but without success message
            errorMsg = "Unexpected server response format";
            //LogToFile("AuthenticateUser: Unexpected response format: " + readBuffer);
            return false;
        }
    }
    catch (const json::exception& e) {
        errorMsg = "Failed to parse success response: " + std::string(e.what());
        //LogToFile("AuthenticateUser: JSON parsing error: " + std::string(e.what()));
        return false;
    }
}
                ...
                if (ImGui::Button(OBF("Login"), ImVec2(130, 30)) && !IsLoggingIn) {
                    ...
                    if (UsernameBuffer[0] == '\0' || PasswordBuffer[0] == '\0') {
                        ...
                        if (AuthenticateUser(UsernameBuffer, PasswordBuffer, hwid, errorMsg)) {
                            IsLoggedIn = true;
                            UserName = UsernameBuffer;
                            // We'll need to get actual expiry from API
            ...
            ImGui::Begin("ClanUtils", nullptr, ImGuiWindowFlags_NoDecoration | ImGuiWindowFlags_NoScrollWithMouse);
            {
                ...
                case Settings:
                {
                    ...
                    case Settings2:
                    {
                        ...
                        ImGui::EndChild();  // Add this missing EndChild()
                        ImGui::SetCursorPos(ImVec2(CHILD_SPACED_X, 165 - TabAnim)); // Use TabAnim here
                ...
                std::string Expiry = "Expiry: " + expiryDate; // Use the UserExpiry variable set during login
                draw->AddText(Fonts::Medium, 16.0f, ImVec2(pos.x + 27, pos.y + 580), ImColor(60, 60, 255), UserName.c_str()); // Use UserName variable
                draw->AddText(Fonts::Medium, 14.0f, ImVec2(pos.x + 27, pos.y + 600), ImColor(255, 255, 255), expiryDate.c_str());
